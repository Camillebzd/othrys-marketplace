...
import { MediaRenderer, ThirdwebNftMedia, Web3Button, useContract, useMinimumNextBid, useNFT, useValidDirectListings, useValidEnglishAuctions } from "@thirdweb-dev/react";
...
export default function TokenPage() {
  ...
  const { data: nft, isLoading: isNFTLoading } = useNFT(nftCollection, tokenId);
  ...
  useEffect(() => {
    const callContractMetadata = async () => setContractMetadata(await nftCollection?.metadata.get());

    if (nftCollection != undefined)
      callContractMetadata();
  }, [nftCollection]);

  return (
    <Container maxW={"1200px"} p={5} my={5}>
        {isNFTLoading || nft === undefined ?
          (
            <Skeleton></Skeleton>
          ) :
          (
              ...
              <Stack spacing={"20px"}>
                ...
                <Box>
                  ...
                  <SimpleGrid columns={2} spacing={4}>
                    {Object.entries<any>(nft?.metadata?.attributes || {}).map(
                      ([key, value]) => (
